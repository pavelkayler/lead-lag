...
export function HedgePage() {
  ...
  <Col md={12}><Card body>
    <Tabs activeKey={tab} onSelect={(k) => setTab(k || "orders")}>
      <Tab eventKey="orders" title={`Открытые ордера (${openOrders.length})`}><div style={{ maxHeight: 260, overflowY: "auto" }}><Table size="sm"><thead><tr><th>symbol</th><th>side</th><th>qty</th><th>price</th><th>linkId</th><th>status</th></tr></thead><tbody>{openOrders.map((o, i) => <tr key={i}><td>{o.symbol}</td><td>{o.side}</td><td>{o.qty || o.leavesQty}</td><td>{o.price || o.triggerPrice}</td><td>{o.orderLinkId || o.order_link_id || "-"}</td><td>{o.orderStatus || o.order_status || "-"}</td></tr>)}</tbody></Table></div></Tab>
