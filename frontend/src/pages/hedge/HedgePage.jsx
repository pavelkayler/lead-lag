...
export function HedgePage() {
  ...
  const openOrders = (app.tradeState?.openOrders || []).filter((o) => !symbol || o.symbol === symbol);
  const positions = (app.tradeState?.positions || []).filter((p) => !symbol || p.symbol === symbol);
  const executions = (app.tradeState?.executions || []).filter((e) => !symbol || e.symbol === symbol);
  ...
  return <Row className="g-3"><Col md={12}><Card body>
    <div className="d-flex gap-2 flex-wrap align-items-end">
      ...
      <div>
        ...
        <Form.Control type="number" value={offsetPercent} onChange={(e)=>setOffsetPercent(Number(e.target.value))} style={{maxWidth:130}} />
      </div>
      <div>
        <Form.Label>Режим TP/SL</Form.Label>
        <Form.Select value={tpSlMode} onChange={(e) => setTpSlMode(e.target.value)} style={{maxWidth:180}}>
          <option value="roiPct">TP/SL по ROI (%)</option>
          <option value="pnlUSDT">TP/SL по PnL (USDT)</option>
        </Form.Select>
      </div>
      <div>
        <Form.Label>{tpLabel}</Form.Label>
        <Form.Control type="number" value={tp} onChange={(e)=>setTp(Number(e.target.value))} style={{maxWidth:130}} />
      </div>
      <div>
        <Form.Label>{slLabel}</Form.Label>
        <Form.Control type="number" value={sl} onChange={(e)=>setSl(Number(e.target.value))} style={{maxWidth:130}} />
      </div>
    ...
    <Tabs activeKey={tab} onSelect={(k) => setTab(k || "orders")}> 
      <Tab eventKey="orders" title={`Открытые ордера (${openOrders.length})`}>
        <div style={{ maxHeight: 260, overflowY: "auto" }}>
          <Table size="sm"><thead><tr><th>symbol</th><th>side</th><th>qty</th><th>price</th></tr></thead><tbody>{openOrders.map((o, i) => <tr key={i}><td>{o.symbol}</td><td>{o.side}</td><td>{o.qty}</td><td>{o.price || o.triggerPrice}</td></tr>)}</tbody></Table>
        </div>
      </Tab>
      ...
      <Tab eventKey="history" title={`История (${executions.length})`}>
        <div style={{ maxHeight: 260, overflowY: "auto" }}>
          <Table size="sm"><thead><tr><th>symbol</th><th>side</th><th>execQty</th><th>execPrice</th><th>execTime</th></tr></thead><tbody>{executions.map((e, i) => <tr key={i}><td>{e.symbol}</td><td>{e.side}</td><td>{e.execQty}</td><td>{e.execPrice}</td><td>{e.execTime ? new Date(Number(e.execTime)).toLocaleTimeString("ru-RU") : "-"}</td></tr>)}</tbody></Table>
        </div>
      </Tab>
    </Tabs>
  </Card></Col></Row>;
