...
const EMPTY = {
  ...
  entryStrictness: 65, useFixedLeaders: false, interExchangeArbitrage: true, interExchangeArbEnabled: true, riskMode: "OFF",
  riskImpulseMargin: 0.4, riskQtyMultiplier: 0.5, riskCooldownMin: 45, maxRiskEntriesPerHour: 1, protectCoreLeaders: true, blacklistSymbols: [], blacklist: [],
...
export function PresetsPage() {
  ...
  return <Row className="g-3">
    ...
    <Col md={5}><Card body><h6>Настройки входа (lead-lag)</h6>
      ...
      <Form.Check className="mb-2" type="checkbox" label="Межбиржевой арбитраж (Bybit ↔ Binance)" checked={draft.interExchangeArbitrage !== false} onChange={(e) => setDraft((d) => ({ ...d, interExchangeArbitrage: e.target.checked, interExchangeArbEnabled: e.target.checked }))} />
      ...
      <Row className="g-2 mb-2"><Col><Form.Label>Режим риска</Form.Label><Form.Select value={draft.riskMode || "OFF"} onChange={(e) => setDraft((d) => ({ ...d, riskMode: e.target.value }))}><option value="OFF">OFF — обычные входы, без риска</option><option value="LOW">LOW — редкие риск-входы при почти пройденном импульсе</option><option value="MED">MED — умеренное расширение окна риска</option><option value="HIGH">HIGH — агрессивнее, только для поиска входов</option></Form.Select></Col><Col><Form.Label>riskImpulseMargin (0.1..0.8)</Form.Label><Form.Control type="number" min={0.1} max={0.8} step="0.05" value={draft.riskImpulseMargin ?? 0.4} onChange={(e) => setDraft((d) => ({ ...d, riskImpulseMargin: Number(e.target.value) }))} /></Col></Row>
      <Row className="g-2 mb-2"><Col><Form.Label>riskQtyMultiplier (0.2..1.0)</Form.Label><Form.Control type="number" min={0.2} max={1} step="0.05" value={draft.riskQtyMultiplier ?? 0.5} onChange={(e) => setDraft((d) => ({ ...d, riskQtyMultiplier: Number(e.target.value) }))} /></Col><Col><Form.Label>riskCooldownMin (минут)</Form.Label><Form.Control type="number" min={1} value={draft.riskCooldownMin ?? 45} onChange={(e) => setDraft((d) => ({ ...d, riskCooldownMin: Number(e.target.value) }))} /></Col></Row>
      <Row className="g-2 mb-2"><Col><Form.Label>maxRiskEntriesPerHour</Form.Label><Form.Control type="number" min={1} value={draft.maxRiskEntriesPerHour ?? 1} onChange={(e) => setDraft((d) => ({ ...d, maxRiskEntriesPerHour: Number(e.target.value) }))} /></Col><Col><Form.Label>targetMinTradesPerHour</Form.Label><Form.Control type="number" value={draft.autoTune?.targetMinTradesPerHour ?? 1} onChange={(e) => setDraft((d) => ({ ...d, autoTune: { ...(d.autoTune || {}), targetMinTradesPerHour: Number(e.target.value) } }))} /></Col></Row>
