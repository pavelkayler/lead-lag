...
const EMPTY = { name: "", qtyUSDT: 25, minCorr: 0.15, impulseZ: 2.2, tpSigma: 1.5, slSigma: 1.0, maxHoldBars: 20, cooldownBars: 20, entryStrictness: 65, useFixedLeaders: false, interExchangeArbEnabled: true, riskMode: "OFF", riskImpulseMargin: 0.4, riskQtyMultiplier: 0.5, riskCooldownMin: 45, maxRiskEntriesPerHour: 1, blacklistSymbols: [], blacklist: [], autoTune: { enabled: true, startTuningAfterMin: 12, tuningIntervalSec: 90, targetMinTradesPerHour: 1, bounds: { minCorr: { floor: 0.05, ceil: 0.4 }, impulseZ: { floor: 1.2, ceil: 4 }, confirmZ: { floor: 0.05, ceil: 1 }, edgeMult: { floor: 1.5, ceil: 8 }, riskImpulseMargin: { floor: 0.1, ceil: 0.8 }, riskQtyMultiplier: { floor: 0.2, ceil: 1.0 }, riskModeMax: 3 } } };
...
export function PresetsPage() {
  ...
  return <Row className="g-3">
    ...
    <Col md={5}><Card body><h6>{draft.name ? "Редактирование" : "Создание"}</h6>
      ...
      <Form.Check className="mb-2" type="checkbox" label="Межбиржевой арбитраж (Bybit ↔ Binance)" checked={draft.interExchangeArbEnabled !== false} onChange={(e) => setDraft((d) => ({ ...d, interExchangeArbEnabled: e.target.checked }))} />

      <h6 className="mt-3">Risk-entry</h6>
      <Row className="g-2 mb-2">
        <Col md={6}><Form.Label>riskMode</Form.Label><Form.Select value={draft.riskMode || "OFF"} onChange={(e) => setDraft((d) => ({ ...d, riskMode: e.target.value }))}><option value="OFF">OFF</option><option value="LOW">LOW</option><option value="MED">MED</option><option value="HIGH">HIGH</option></Form.Select></Col>
        <Col md={6}><Form.Label>riskImpulseMargin</Form.Label><Form.Control type="number" step="0.1" value={draft.riskImpulseMargin ?? 0.4} onChange={(e) => setDraft((d) => ({ ...d, riskImpulseMargin: Number(e.target.value) }))} /></Col>
        <Col md={6}><Form.Label>riskQtyMultiplier</Form.Label><Form.Control type="number" step="0.05" value={draft.riskQtyMultiplier ?? 0.5} onChange={(e) => setDraft((d) => ({ ...d, riskQtyMultiplier: Number(e.target.value) }))} /></Col>
        <Col md={6}><Form.Label>riskCooldownMin</Form.Label><Form.Control type="number" value={draft.riskCooldownMin ?? 45} onChange={(e) => setDraft((d) => ({ ...d, riskCooldownMin: Number(e.target.value) }))} /></Col>
        <Col md={12}><Form.Label>maxRiskEntriesPerHour</Form.Label><Form.Control type="number" value={draft.maxRiskEntriesPerHour ?? 1} onChange={(e) => setDraft((d) => ({ ...d, maxRiskEntriesPerHour: Number(e.target.value) }))} /></Col>
      </Row>
      ...
      <Row className="g-2 mb-2">
        ...
        <Col md={6}><Form.Label>Пределы riskImpulseMargin (мин/макс)</Form.Label><div className="d-flex gap-1"><Form.Control type="number" step="0.1" value={draft.autoTune?.bounds?.riskImpulseMargin?.floor ?? 0.1} onChange={(e) => setDraft((d) => ({ ...d, autoTune: { ...(d.autoTune || {}), bounds: { ...(d.autoTune?.bounds || {}), riskImpulseMargin: { ...(d.autoTune?.bounds?.riskImpulseMargin || {}), floor: Number(e.target.value) } } } }))} /><Form.Control type="number" step="0.1" value={draft.autoTune?.bounds?.riskImpulseMargin?.ceil ?? 0.8} onChange={(e) => setDraft((d) => ({ ...d, autoTune: { ...(d.autoTune || {}), bounds: { ...(d.autoTune?.bounds || {}), riskImpulseMargin: { ...(d.autoTune?.bounds?.riskImpulseMargin || {}), ceil: Number(e.target.value) } } } }))} /></div></Col>
        <Col md={6}><Form.Label>Пределы riskQtyMultiplier (мин/макс)</Form.Label><div className="d-flex gap-1"><Form.Control type="number" step="0.05" value={draft.autoTune?.bounds?.riskQtyMultiplier?.floor ?? 0.2} onChange={(e) => setDraft((d) => ({ ...d, autoTune: { ...(d.autoTune || {}), bounds: { ...(d.autoTune?.bounds || {}), riskQtyMultiplier: { ...(d.autoTune?.bounds?.riskQtyMultiplier || {}), floor: Number(e.target.value) } } } }))} /><Form.Control type="number" step="0.05" value={draft.autoTune?.bounds?.riskQtyMultiplier?.ceil ?? 1} onChange={(e) => setDraft((d) => ({ ...d, autoTune: { ...(d.autoTune || {}), bounds: { ...(d.autoTune?.bounds || {}), riskQtyMultiplier: { ...(d.autoTune?.bounds?.riskQtyMultiplier || {}), ceil: Number(e.target.value) } } } }))} /></div></Col>
        <Col md={12}><Form.Label>Максимальный riskMode для авто-тюна (0..3)</Form.Label><Form.Control type="number" min={0} max={3} value={draft.autoTune?.bounds?.riskModeMax ?? 3} onChange={(e) => setDraft((d) => ({ ...d, autoTune: { ...(d.autoTune || {}), bounds: { ...(d.autoTune?.bounds || {}), riskModeMax: Number(e.target.value) } } }))} /></Col>
      </Row>
